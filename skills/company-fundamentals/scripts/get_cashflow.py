...
def get_cashflow(stock_code: str, years: int = 5, frequency: str = 'both', output_dir: Path = None) -> dict:
    """
    获取股票的现金流量表数据。
    ...
    logger.info(f"正在获取 {stock_info['name']} ({stock_code}) 的现金流量表，{years} 年，频率: {frequency}")
    ...
    try:
        ...
        if not cashflow_df.empty:
            cashflow_df = clean_financial_data(cashflow_df)
            
            # 年度数据（12月31日的报告）
            if fetch_yearly:
                yearly_data = [r for r in cashflow_df.to_dict('records') 
            ...
            # 季度数据
            if fetch_quarterly:
                quarterly_data = cashflow_df.head(years * 4).to_dict('records')
    ...
    for key, data in result.items():
        ...
        logger.info(f"已保存 {key} 现金流量表到 {output_file}")
...
if __name__ == '__main__':
    ...
    get_cashflow(args.stock_code, args.years, args.frequency, args.output_dir)
