...
def main():
    ...
    if cashflow_data:
        ...
        print(f"经营活动产生的现金流量净额: {(latest.get('经营活动产生的现金流量净额') or 0) / 10000:.2f} 万元")
        print(f"投资活动产生的现金流量净额: {(latest.get('投资活动产生的现金流量净额') or 0) / 10000:.2f} 万元")
        print(f"筹资活动产生的现金流量净额: {(latest.get('筹资活动产生的现金流量净额') or 0) / 10000:.2f} 万元")
        print(f"销售商品提供劳务收到的现金: {(latest.get('销售商品提供劳务收到的现金') or 0) / 10000:.2f} 万元")
        print(f"支付给职工以及为职工支付的现金: {(latest.get('支付给职工以及为职工支付的现金') or 0) / 10000:.2f} 万元")
        print(f"购建固定资产无形资产和其他长期资产支付的现金: {(latest.get('购建固定资产无形资产和其他长期资产支付的现金') or 0) / 10000:.2f} 万元")
        print(f"分配股利利润或偿付利息支付的现金: {(latest.get('分配股利利润或偿付利息支付的现金') or 0) / 10000:.2f} 万元")
