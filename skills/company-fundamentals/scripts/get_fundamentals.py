...
def get_daily_basic(pro, ts_code):
    ...
    try:
        ...
        return {
            ...
            'dv_ratio': safe_float(latest.get('dv_ratio')),
            'total_mv': safe_float(latest.get('total_mv')),
            'circ_mv': safe_float(latest.get('circ_mv')),
...
def get_financial_indicators(pro, ts_code, start_date, end_date):
    """获取财务指标数据"""
    try:
        ...
        return {
            ...
            'op_yoy': safe_float(latest.get('op_yoy')),
            'dt_netprofit_yoy': safe_float(latest.get('dt_netprofit_yoy')),
            'or_yoy': safe_float(latest.get('or_yoy')),
...
def get_income_summary(pro, ts_code, start_date, end_date):
    """获取利润表摘要"""
    try:
        df = pro.income(
            ts_code=ts_code,
...
def get_balance_sheet_summary(pro, ts_code, start_date, end_date):
    """获取资产负债表摘要"""
    try:
        df = pro.balancesheet(
            ts_code=ts_code,
...
def get_cashflow_summary(pro, ts_code, start_date, end_date):
    """获取现金流量表摘要"""
    try:
        df = pro.cashflow(
            ts_code=ts_code,
...
def main():
    ...
    print("获取财务指标...")
    financial_indicators = get_financial_indicators(pro, ts_code, args.start_date, args.end_date)

    print("获取利润表摘要...")
    income_summary = get_income_summary(pro, ts_code, args.start_date, args.end_date)

    print("获取资产负债表摘要...")
    balance_summary = get_balance_sheet_summary(pro, ts_code, args.start_date, args.end_date)

    print("获取现金流量表摘要...")
    cashflow_summary = get_cashflow_summary(pro, ts_code, args.start_date, args.end_date)
