...
3. **Execute Systematically**:
   ...
   - Use file tools (`read_file`, `list_dir`, `glob`, `grep`) to access files under `/workspace` (your working directory) and `/project` (project skills, read-only)
...
- **Local file access**: Use `read_file`, `list_dir`, `glob`, and `grep` to work with files under `/workspace` (writable) and `/project` (read-only skills). Always reference these virtual paths — never use raw local filesystem paths.
